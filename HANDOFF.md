...
- Top row at 26px / 36px inset: left = logo mark (28×31 vector) + `/ PAGE NAME` label (desktop only); center = blinking 7px dot + `REC` + live timecode `HH:MM:SS:FF` (desktop only, 250ms tick); right = `Get a quote` (underlined) + `Menu ≡` (mobile only).
- Bottom-left (desktop): `ISO 800 · f/1.8 · 1/50 · EV −1.0`. Bottom-right (desktop): `FR 0000 / 1000` = scroll progress ×1000.
- Desktop only: vertical nav rail fixed right 36px, vertically centered: 5 page labels, each with a trailing 1px tick (12px, 28px when active) and opacity .5/1.
...
- Back-to-top: 44px round outline button, fixed right 36px bottom 60px, shows after 1 viewport of scroll.
...
- Mobile dock (mobile only): fixed bottom 16px, 3 columns: `WhatsApp` (filled pill → `https://wa.me/16478391491`), `Call` (outline pill → `tel:+16478391491`), round `→` (Get a quote). Height 48px.
- Mobile menu (full-screen `#0c0c0c` overlay): list of 5 pages (number 01–05 + Cormorant 28–44px), bottom row = horizontal lockup logo + `Close ✕` pill.
